  ...
  const card = (
    <>
      <div className="relative overflow-hidden bg-neutral-100">
        {product.sale ? (
          <span
            className={`absolute left-3 top-3 z-10 px-2 py-1 text-[10px] font-semibold tracking-wider text-white ${variant.accentBg}`}
          >
            SALE
          </span>
        ) : null}
        ...
        <div className="absolute inset-x-0 bottom-0 translate-y-full bg-white/95 p-3 transition group-hover:translate-y-0">
          {product.href ? (
            <span
              className={`block w-full border py-2 text-center text-xs font-semibold tracking-wider ${variant.accentText} border-current`}
            >
              VIEW PRODUCT
            </span>
          ) : (
            <button
              type="button"
              className={`w-full border py-2 text-xs font-semibold tracking-wider ${variant.accentText} border-current`}
            >
              QUICK VIEW
            </button>
          )}
