...
export function FurnitureHomepage({ data }: { data: Homepage }) {
  ...
  return (
    <>
      ...
      <section className={`border-b border-neutral-200 ${variant.featureBg}`}>
          ...
          {data.bestSellerTabs.map((tab, index) => (
            <button
              ...
              className={`text-xs font-semibold tracking-[0.15em] ${
                index === 0 ? variant.accentText : "text-neutral-400"
              }`}
            >
              {tab}
            </button>
        ...
        <section key={section.title} className="mx-auto max-w-7xl px-6 py-16">
          <div className="max-w-2xl">
            ...
            <a
              ...
              className={`mt-6 inline-block border px-6 py-2 text-xs font-semibold tracking-[0.15em] transition ${variant.accentText} border-current ${variant.accentHover}`}
            >
              {section.ctaLabel}
            </a>
          </div>
      ...
      <section className={`py-20 text-center ${variant.featureBg}`}>
        <div className="mx-auto max-w-2xl px-6">
          <h2 className="text-3xl font-light text-neutral-900">{data.ctaBanner.title}</h2>
          <p className="mt-4 text-sm leading-relaxed text-neutral-600">{data.ctaBanner.body}</p>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-6 py-16">
        <h2 className="text-center text-2xl font-light text-neutral-900">What People Say</h2>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 text-center">
        ...
        <p className="mx-auto mt-3 max-w-lg text-sm text-neutral-600">{data.instagram.subtitle}</p>
        <a
          href={data.instagram.ctaHref}
          className={`mt-6 inline-block border px-8 py-3 text-xs font-semibold tracking-[0.15em] transition ${variant.accentText} border-current ${variant.accentHover}`}
        >
          {data.instagram.ctaLabel}
        </a>
      </section>

      <section className={`py-16 text-center text-white ${variant.accentBg}`}>
        <h2 className="mx-auto max-w-xl text-xl font-light">{data.newsletter.title}</h2>
        <button
          type="button"
